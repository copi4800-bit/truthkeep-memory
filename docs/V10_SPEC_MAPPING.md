...
This document maps the formal v10 theory notes to the current runtime implementation.
...
The current runtime is behaviorally aligned with the v10 model:
...
| Continuous state vector `z_i(t) = [c, u, d, kappa, r]^T` | `/home/hali/.openclaw/11.md` section 4.3 and `/home/hali/.openclaw/12.md` section 2.1 | Runtime computes `belief_score`, `usage_signal`, `decay_signal`, `conflict_signal`, `readiness_score` in `aegis_py/retrieval/v10_dynamics.py` | `Approx` |
...
- `aegis_py/retrieval/v10_dynamics.py`: v10 dynamic signals, transition gate, feedback, energy, reason tags
- `aegis_py/v10/state_machine.py`: discrete state transitions
- `aegis_py/storage/evidence.py`: evidence events and artifacts
- `tests/benchmarks/test_v10_dynamics_sli.py`: benchmark gate for v10 behavior

## Practical Reading

The current codebase should be interpreted as:

- a practical v10 runtime with strong behavioral alignment
...
If the project wants stronger formal fidelity later, the next step is not a rewrite. The next step is to define and persist a canonical v10 state contract per memory, then progressively migrate retrieval, hygiene, and governance to consume that contract directly.
