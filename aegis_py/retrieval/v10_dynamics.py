...
from aegis_py.config.features import ENABLE_BAYES
...
def _belief_score(
    *,
    ...
    legacy_belief = clamp01(prior_belief + increased - decreased)
    # Phase 3 — Bayesian posterior blended with legacy for backward compatibility
    if ENABLE_BAYES:
        bayesian_posterior = BayesianBeliefEngine.belief_from_signals(
            prior_belief=prior_belief,
            evidence_signal=evidence_signal,
            support_signal=support_signal,
            conflict_signal=conflict_signal,
        )
        return round(bayesian_posterior * 0.7 + legacy_belief * 0.3, 6)
    else:
        return round(legacy_belief, 6)
...
def bundle_energy_snapshot(
    ...
    *,
    ...
    energy = round(max(0.0, sum(energy_terms) / bundle_size), 6)
    objective = round(
        retrieval_loss
