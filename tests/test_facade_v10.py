...
class TestFacadeV10(unittest.TestCase):
    def setUp(self):
        ...
        self.db_path = str(Path(tempfile.gettempdir()) / f"test_aegis_facade_v10_{uuid.uuid4().hex}.db")
    ...
    def test_patch_c_wording_logic(self):
        ...
        self.assertEqual(results2[0].get("truth_role"), "winner")

        winner = results2[0].get("memory", {})
        winner_id = winner.get("id")
        winner_subject = winner.get("subject")
        rows = self.aegis._app.storage.fetch_all(
            "SELECT id, status, metadata_json FROM memories WHERE subject = ? ORDER BY created_at",
            (winner_subject,),
        )
        self.assertEqual(len(rows), 2)
        superseded = [row for row in rows if row["id"] != winner_id]
        self.assertEqual(superseded[0]["status"], "superseded")
        meta = self.aegis._app.storage._coerce_metadata(superseded[0]["metadata_json"])
        self.assertFalse(meta.get("is_winner", False), f"Superseded memory kept winner metadata: {meta}")
