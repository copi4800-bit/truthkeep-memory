...
def main() -> int:
    ...
    output = repo_root / ".planning" / "benchmarks" / "consumer_shell_report.html"
    output.parent.mkdir(parents=True, exist_ok=True)

    with tempfile.TemporaryDirectory(prefix="truthkeep_consumer_shell_html_") as tmp:
        db_path = str(Path(tmp) / "consumer_shell_html.db")
        ...
        try:
            old_stored = app.put_memory(
                ...
                type="semantic",
                scope_type="agent",
                scope_id="consumer_shell_html_demo",
                source_kind="manual",
    ...
    output.write_text(render_consumer_shell_html(payload), encoding="utf-8")
