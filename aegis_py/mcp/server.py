...
def main() -> None:
    ...
    try:
        ...
        if args.test:
            ...
            try:
                from aegis_py.main import mcp
                if mcp is not None:
                    import sys
                    sys.stderr.write("Starting real FastMCP stdio server...\n")
                    mcp.run()
                    return
            except Exception as me:
                import sys
                sys.stderr.write(f"Failed to start FastMCP server: {me}. Falling back to sleep loop.\n")
