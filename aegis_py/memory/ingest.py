    ...
    def ingest(
        ...
        **kwargs
        ...
        if metadata.get("is_correction"):
            ...
            retired_ids = list(all_ids_to_retire)
            ...
            # Phase 3 — Backpropagation: propagate correction adjustments to linked memories
            if features.ENABLE_BACKPROP:
                try:
                    ...
                    from aegis_py.hygiene.transitions import now_iso
                    import uuid

                    corrected_ids = list(all_ids_to_retire)
                    for corrected_id in corrected_ids:
                        # Sử dụng expand_graph để tìm toàn bộ node bị ảnh hưởng lên tới 3 hops trong cùng scope
                        impacted_links = expand_graph(
                            self.storage,
                            seed_ids=[corrected_id],
                            mode=TraversalMode.IMPACT_ANALYSIS,
                            max_hops=3,
                            scope_type=scope_type,
                            scope_id=scope_id,
                            limit=50
                        ...
                        if not impacted_links:
                            continue

                        # BFS xây dựng cây để xác định depth và parent_weight chính xác từ corrected_id
                        queue = [(corrected_id, 0, 1.0, "depends_on")]  # (node_id, depth, link_weight, link_type)
                        visited = {corrected_id}
                        linked_memories = []

                        while queue:
                            curr, depth, p_weight, p_type = queue.pop(0)
                            if depth > 0:
                            ...
                            for link in impacted_links:
                                # Trong impact_analysis, direction = backward, nghĩa là target_id là node cha (curr)
                                # và neighbor_id (source_id) là node con
                                if link["target_id"] == curr and link["neighbor_id"] not in visited:
                                    visited.add(link["neighbor_id"])
                                    queue.append((link["neighbor_id"], depth + 1, link["weight"], link["link_type"]))
                        ...
                        for adj in adjustments:
                            # Lấy old confidence
                            old_row = self.storage.fetch_one("SELECT confidence FROM memories WHERE id = ?", (adj["memory_id"],))
                            if not old_row:
                                continue
                            old_conf = old_row["confidence"]
                            new_conf = max(0.1, old_conf + adj["confidence_delta"])

                            # Cập nhật DB
                            self.storage.execute(
                                "UPDATE memories SET confidence = ? WHERE id = ?",
                                (new_conf, adj["memory_id"]),
                            )

                            # Ghi nhật ký graph_adjustment_audit
                            link_type = "depends_on"
                            for lm in linked_memories:
                                if lm["memory_id"] == adj["memory_id"]:
                            ...
                            self.storage.execute(
                                """
                                INSERT INTO graph_adjustment_audit (
                                    id, source_memory_id, target_memory_id, relation_type, reason,
                                    old_confidence, new_confidence, delta, depth, link_weight, created_at
                                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                                """,
                                (
                                    audit_id,
                                    ...
                                    adj["memory_id"],
                                    link_type,
                                    "correction_backpropagation",
                                    old_conf,
                                    new_conf,
                                    adj["confidence_delta"],
                                    adj["depth"],
                                    adj["link_weight"],
                                    now_iso(),
                                )
                    ...
                    pass  # Backprop must never block the main correction flow
        ...
        if success:
            # Tạo liên kết superseded_by đầy đủ cho các ký ức bị thay thế để bảo toàn Invariant
            if metadata.get("is_correction") and 'retired_ids' in locals() and retired_ids:
                import uuid
                from datetime import datetime, timezone
                for oid in retired_ids:
                    link_id = f"link:{uuid.uuid4().hex[:8]}"
                    created_at = datetime.now(timezone.utc).isoformat()
                    try:
                        self.storage.execute(
                            """
                            INSERT INTO memory_links (id, source_id, target_id, link_type, weight, metadata_json, created_at)
                            VALUES (?, ?, ?, ?, ?, ?, ?)
                            """,
                            (link_id, oid, memory.id, "superseded_by", 1.0, "{}", created_at)
                        )
                    except Exception:
                        pass  # Tránh làm nghẽn luồng ghi chính nếu xảy ra lỗi ghi liên kết trùng lặp
