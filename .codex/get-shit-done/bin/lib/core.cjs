...
function findProjectRoot(startDir) {
  ...
  // Prefer the current directory when it already looks like a standalone GSD
  // project root. This matters for nested repos that live inside a larger
  // workspace which also has its own .planning/. Without this guard, the
  // parent workspace incorrectly steals command-driven GSD operations.
  const localPlanning = path.join(resolved, '.planning');
  const localGit = path.join(resolved, '.git');
  if (
    fs.existsSync(localPlanning) &&
    fs.statSync(localPlanning).isDirectory() &&
    fs.existsSync(localGit)
  ) {
    return resolved;
  }
