...
from ..graph.relation_types import validate_relation_type, RelationType
...
class GraphRepository:
    ...
    def upsert_memory_link(
        ...
        *,
        ...
        # Chuẩn hóa và xác thực Relation Type nghiêm ngặt
        norm_type = link_type.lower()
        if not validate_relation_type(norm_type):
            canonical = canonical_edge_type(link_type)
            if validate_relation_type(canonical):
                link_type = canonical
            else:
                raise ValueError(f"Invalid relation type: {link_type}")
        else:
            link_type = norm_type
    ...
    def count_links_for_memory(self, memory_id: str) -> int:
        """Count total links for a memory (Euler degree). Used by centrality bonus."""
        row = self.storage.fetch_one(
