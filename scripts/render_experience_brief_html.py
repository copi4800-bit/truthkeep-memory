...
def main() -> int:
    ...
    output = repo_root / ".planning" / "benchmarks" / "experience_brief_report.html"
    output.parent.mkdir(parents=True, exist_ok=True)

    with tempfile.TemporaryDirectory(prefix="truthkeep_experience_brief_html_") as tmp:
        db_path = str(Path(tmp) / "experience_brief_html.db")
        ...
        try:
            old_stored = app.put_memory(
                ...
                type="semantic",
                scope_type="agent",
                scope_id="experience_brief_html_demo",
                source_kind="manual",
    ...
    output.write_text(render_experience_brief_html(payload), encoding="utf-8")
