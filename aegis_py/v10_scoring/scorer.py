...
class ResidualScorer:
    ...
    def score(self, memory: MemoryRecordV10, query_signals: Dict[str, float], intent: str = "normal_recall") -> JudgmentTrace:
        ...
        w_graph = self.config.get("graph", {}).get("w_graph", 0.20)
        graph_bonus = w_graph * math.tanh(memory.graph_score)
        trace.factors["graph_bonus"] = graph_bonus

        pre_constraint_score = trace.base_score + trace.judge_delta + trace.life_delta + graph_bonus
